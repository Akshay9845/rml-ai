...
class UltraFastFolderProcessor {
private:
    std::string data_dir;
...
public:
    UltraFastFolderProcessor(const std::string& data_dir, const std::string& output_dir) 
    ...
    size_t processFile(const std::string& filepath, const std::string& folder_name, 
        ...
        while (std::getline(file, line)) {
            ...
            try {
                ...
                if (record_id.empty()) record_id = json_data["doc_id"];
    ...
    void processAllFolders() {
        std::cout << "🚀 Ultra-Fast Folder-by-Folder RML Processing" << std::endl;
        ...
        std::cout << "   Average speed: " << std::fixed << std::setprecision(1) 
                  << (355.0 * 1024 / total_time.count()) << " MB/s" << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        ...
        std::cout << "Example: " << argv[0] << " data/ output/folder_processing/" << std::endl;
        return 1;
    }
    ...
    UltraFastFolderProcessor processor(data_dir, output_dir);
