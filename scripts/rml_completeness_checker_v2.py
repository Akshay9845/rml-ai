#!/usr/bin/env python3
"""
RML Completeness Checker V2 - Improved
Searches across all chunk files for the same record_id and assembles complete RML records
"""
...
class RMLCompletenessCheckerV2:
    ...
    def find_all_component_files(self):
        """Find all component files in the directory"""
        logger.info(f"🔍 Finding all component files in: {self.data_dir}")
    ...
    def read_concepts_line(self, concepts_files, line_number=1):
        """Read a specific line from concepts files"""
        logger.info(f"📖 Reading line {line_number} from concepts files...")
                    ...
                    for i, line in enumerate(f, 1):
                        if i == line_number:
                            ...
                            if line:
                                ...
                                logger.info(f"  ✅ Read concepts data from {os.path.basename(concepts_file)}: {data}")
                                return data, concepts_file
                            else:
                                logger.warning(f"  ⚠️ Line {line_number} is empty in {concepts_file}")
                                continue
            except Exception as e:
                logger.error(f"  ❌ Error reading {concepts_file}: {e}")
                continue
        
        logger.error("  ❌ Could not read concepts data from any file")
        return None, None
    
    def find_record_in_all_files(self, component_files, record_id, chunk=1):
        """Find a specific record in all files of a component type"""
        found_data = None
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        data = json.loads(line)
                        if data.get('record_id') == record_id and data.get('chunk', 1) == chunk:
                            found_data = data
                            logger.info(f"    ✅ Found in {os.path.basename(file_path)}")
                            break
    ...
    def check_record_completeness(self, record_id, chunk, component_files):
        """Check if a record has all 10 RML components across all files"""
        logger.info(f"🔍 Checking completeness for record_id: {record_id}, chunk: {chunk}")
        
        found_components = {}
        missing_components = []
        
        # Check each component across all its files
        for component in self.rml_components:
            if component in component_files:
                data = self.find_record_in_all_files(component_files[component], record_id, chunk)
                if data:
    ...
    def save_complete_record(self, complete_record, output_file="data/complete_rml_sample_v2.jsonl"):
    ...
    def run_single_record_check(self, line_number=1):
        ...
        logger.info("🚀 Starting RML Completeness Check V2...")
        self.print_system_status()
        
        # Find all component files
        component_files = self.find_all_component_files()
        
        if 'concepts' not in component_files:
            logger.error("❌ Concepts files not found!")
            return
        
        # Read concepts line
        concepts_data, concepts_file = self.read_concepts_line(component_files['concepts'], line_number)
        ...
        # Check completeness across all files
        found_components, missing_components = self.check_record_completeness(record_id, chunk, component_files)
        ...
        logger.info(f"📁 Complete record saved to: data/complete_rml_sample_v2.jsonl")
        
        return complete_record

def main():
    checker = RMLCompletenessCheckerV2()
