...
public:
    bool parseObject(std::unordered_map<std::string, std::string>& result) {
        ...
        while (pos < buffer.length() && buffer[pos] != '}') {
            ...
            // Parse value (handle both string and array)
            if (buffer[pos] == '"') {
                // String value
                pos++; // skip "
                ...
                // Array value - convert to string
                pos++; // skip [
                size_t arrayStart = pos;
                while (pos < buffer.length() && buffer[pos] != ']') pos++;
                if (pos >= buffer.length()) break;
                std::string arrayStr = buffer.substr(arrayStart, pos - arrayStart);
                pos++; // skip ]
                result[key] = arrayStr;
            }
...
private:
    ...
    // Only process REAL RML data directories
            ...
            for (const auto& entry : fs::recursive_directory_iterator(dataset_path)) {
                if (entry.is_regular_file()) {
                    ...
                    if (ext == ".jsonl" || ext == ".json") {
                        // Skip very small files and non-RML files
                        if (fs::file_size(entry.path()) < 1024) continue;
                        
                        // Skip files that are clearly not RML data
                        std::string filename = entry.path().filename().string();
                        if (filename.find("sample_") != std::string::npos) continue;
                        if (filename.find("progress") != std::string::npos) continue;
                        if (filename.find(".done") != std::string::npos) continue;
